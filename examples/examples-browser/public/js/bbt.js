...
function _renderFaceImageSelectList(selectListId, onChange, initialValue) {
  ...
  function renderChildren(select) {
    classes.forEach(className => {
      ...
      indices.forEach(imageIdx =>
        renderOption(optgroup, `${className} ${imageIdx}`, getFaceImageUri(className, imageIdx))
      );
    });
  }
...
async function _createBbtFaceMatcher(numImagesForTraining = 1) {
