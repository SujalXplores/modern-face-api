...
async function _loadImageFromUrl(_url) {
  const img = await requestExternalImage($('#imgUrlInput').val());
  $('#inputImg').get(0).src = img.src;
  updateResults();
}

async function _loadImageFromUpload() {
...
function _initImageSelectionControls(initialValue = 'bbt1.jpg', withFaceExpressionImages = false) {
