import { type IPoint, Point } from '../classes';
import { FaceExpressions } from '../faceExpressionNet';
import { isWithFaceDetection } from '../factories/WithFaceDetection';
import { isWithFaceExpressions, type WithFaceExpressions } from '../factories/WithFaceExpressions';
import { round } from '../utils';
import { DrawTextField } from './DrawTextField';

export type DrawFaceExpressionsInput = FaceExpressions | WithFaceExpressions<object>;
...
export function drawFaceExpressions(
  ...
  faceExpressionsArray.forEach(e => {
    const expr =
      e instanceof FaceExpressions ? e : isWithFaceExpressions(e) ? e.expressions : undefined;
    if (!expr) {
      throw new Error(
