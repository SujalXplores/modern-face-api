...
import { type ISsdMobilenetv1Options, SsdMobilenetv1Options } from './SsdMobilenetv1Options';
...
export class SsdMobilenetv1 extends NeuralNetwork<NetParams> {
  ...
  public async locateFaces(
    ...
    const results = indices.map(idx => {
      const [top, bottom] = [Math.max(0, boxesData[idx][0]), Math.min(1.0, boxesData[idx][2])].map(
        val => val * padY
      );
      const [left, right] = [Math.max(0, boxesData[idx][1]), Math.min(1.0, boxesData[idx][3])].map(
        val => val * padX
      );
