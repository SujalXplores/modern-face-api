...
function extractorsFactory(extractWeights: ExtractWeightsFunction, paramMappings: ParamMapping[]) {
  function extractFilterValues(
    ...
    return tf.tidy(() =>
      tf.transpose(tf.tensor4d(weights, [numFilters, depth, filterSize, filterSize]), [2, 3, 1, 0])
    );
  }
...
export function extractParams(weights: Float32Array): {
  ...
  const { extractConvLayerParams, extractResidualLayerParams } = extractorsFactory(
    extractWeights,
    paramMappings
  );
