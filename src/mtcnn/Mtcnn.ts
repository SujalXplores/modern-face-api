...
import { type NetInput, type TNetInput, toNetInput } from '../dom';
...
export type MtcnnStats = {
  ...
  stage2_extractImagePatches?: number;
  stage2_rnet?: number;
  stage2_nms?: number;
  total_stage2?: number;
  stage3_numInputBoxes?: number;
  stage3_extractImagePatches?: number;
  stage3_onet?: number;
  stage3_nms?: number;
  total_stage3?: number;
};
...
import type { MtcnnResult, NetParams } from './types';
...
export class Mtcnn extends NeuralNetwork<NetParams> {
  ...
  public async forwardInput(
    ...
    const { minFaceSize, scaleFactor, maxNumScales, scoreThresholds, scaleSteps } =
      ...
      extendWithFaceLandmarks(
        ...
        new FaceLandmarks5(
          out3.points[idx].map(pt =>
            pt.sub(new Point(box.left, box.top)).div(new Point(box.width, box.height))
          ),
