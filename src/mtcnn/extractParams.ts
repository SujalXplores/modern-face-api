...
import {
  type ExtractWeightsFunction,
  extractConvParamsFactory,
  extractFCParamsFactory,
  extractWeightsFactory,
  type ParamMapping,
} from '../common';
import type { NetParams, ONetParams, PNetParams, RNetParams, SharedParams } from './types';
...
export function extractParams(weights: Float32Array): {
  ...
  const { extractPNetParams, extractRNetParams, extractONetParams } = extractorsFactory(
    extractWeights,
    paramMappings
  );
