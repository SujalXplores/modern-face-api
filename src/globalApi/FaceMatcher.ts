...
import {
  ...
  type LabeledFaceDescriptorsJSON,
} from '../classes/LabeledFaceDescriptors';
import { euclideanDistance } from '../euclideanDistance';
import type { WithFaceDescriptor } from '../factories';

export interface FaceMatcherJSON {
  distanceThreshold: number;
  labeledDescriptors: LabeledFaceDescriptorsJSON[];
}
...
export class FaceMatcher {
  ...
  constructor(
    inputs:
      ...
      | Float32Array
      | Array<LabeledFaceDescriptors | WithFaceDescriptor<object> | Float32Array>,
  ...
  public matchDescriptor(queryDescriptor: Float32Array): FaceMatch {
    return this.labeledDescriptors
      .map(
        ({ descriptors, label }) =>
          new FaceMatch(label, this.computeMeanDistance(queryDescriptor, descriptors))
      )
  ...
  public static fromJSON(json: FaceMatcherJSON): FaceMatcher {
    const labeledDescriptors = json.labeledDescriptors.map((ld: LabeledFaceDescriptorsJSON) =>
      LabeledFaceDescriptors.fromJSON(ld)
    );
