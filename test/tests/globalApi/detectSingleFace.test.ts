import {
  detectSingleFace,
  type WithAge,
  type WithFaceDescriptor,
  type WithFaceDetection,
  ...
  describeWithNets('detectSingleFace', withNetArgs, () => {
    describe('without face alignment', () => {
      it('detectSingleFace.withFaceExpressions()', async () => {
        const result = await detectSingleFace(imgEl, faceDetectorOptions).withFaceExpressions();

        expectFaceExpressions(result);
      });

      it('detectSingleFace.withAgeAndGender()', async () => {
        const result = await detectSingleFace(imgEl, faceDetectorOptions).withAgeAndGender();
