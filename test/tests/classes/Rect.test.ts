  ...
  describe('constructor', () => {
    ...
    it('throws if coordinates are invalid', () => {
      const expectConstructorToThrow = (
        ...
        expect(
          () => new Rect(x as number, y as number, width as number, height as number)
        ).toThrowError(
          `Box.constructor - expected box to be IBoundingBox | IRect, instead have ${JSON.stringify({ x, y, width, height })}`
        );
      };
    ...
    it('throws if height or width invalid', () => {
      expect(() => new Rect(0, 10, -20, 30, false)).toThrowError(
        'Box.constructor - width (-20) and height (30) must be positive numbers'
      );
      expect(() => new Rect(0, 10, 20, -30, false)).toThrowError(
        'Box.constructor - width (20) and height (-30) must be positive numbers'
      );
