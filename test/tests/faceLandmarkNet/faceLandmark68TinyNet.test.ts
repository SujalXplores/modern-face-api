...
describeWithBackend('faceLandmark68TinyNet', () => {
  ...
  beforeAll(async () => {
    ...
    faceLandmarkPositions1 = await getTestEnv().loadJson<Point[]>(
      'test/data/faceLandmarkPositions1Tiny.json'
    );
    faceLandmarkPositions2 = await getTestEnv().loadJson<Point[]>(
      'test/data/faceLandmarkPositions2Tiny.json'
    );
    faceLandmarkPositionsRect = await getTestEnv().loadJson<Point[]>(
