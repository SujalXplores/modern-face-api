...
import { createCanvasFromMedia, TinyFaceDetectorOptions } from '../../../src';
...
describe('tinyFaceDetector - node', () => {
  ...
  beforeAll(async () => {
    imgTensor = tf.browser.fromPixels(
      createCanvasFromMedia(await getTestEnv().loadImage('test/images/faces.jpg'))
    );
  ...
  describeWithNets('globalApi, tensor inputs', { withAllFacesTinyFaceDetector: true }, () => {
    ...
    it('detectAllFaces.withFaceLandmarks()', async () => {
      ...
      const results = await faceapi.detectAllFaces(imgTensor, options).withFaceLandmarks();
