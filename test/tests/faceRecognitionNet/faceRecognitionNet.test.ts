...
describeWithBackend('faceRecognitionNet', () => {
  ...
  beforeAll(async () => {
    ...
    imgElRect = createCanvasFromMedia(
      await getTestEnv().loadImage('test/images/face_rectangular.png')
    );
  ...
  describeWithNets(
    ...
    ({ faceRecognitionNet }) => {
      it('computes face descriptors for batch of image elements', async () => {
        ...
        const faceDescriptors = [faceDescriptor1, faceDescriptor2, faceDescriptorRect];
      ...
      describe('forwardInput', () => {
        ...
        it('single batch size 1 tf.Tensor4Ds', async () => {
          const tensor = tf.tidy(() =>
            tf.expandDims(tf.browser.fromPixels(imgEl1), 0)
          ) as tf.Tensor4D;
        ...
        it('multiple batch size 1 tf.Tensor4Ds', async () => {
          const tensors = [imgEl1, imgEl1, imgEl1].map(el =>
            tf.tidy(() => tf.expandDims(tf.browser.fromPixels(el), 0))
          ) as tf.Tensor4D[];
