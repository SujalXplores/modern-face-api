...
describe('toNetInput', () => {
  ...
  describe('valid args', () => {
    ...
    it('from HTMLImageElement array', async () => {
      const netInput = await toNetInput([imgEl, imgEl]);
      expect(netInput instanceof NetInput).toBe(true);
      expect(netInput.batchSize).toEqual(2);
    });

    it('from HTMLCanvasElement array', async () => {
      const netInput = await toNetInput([canvasEl, canvasEl]);
      expect(netInput instanceof NetInput).toBe(true);
      expect(netInput.batchSize).toEqual(2);
    });

    it('from mixed media array', async () => {
      const netInput = await toNetInput([imgEl, canvasEl, canvasEl]);
      expect(netInput instanceof NetInput).toBe(true);
  ...
  describe('invalid args', () => {
    it('undefined', async () => {
      ...
      try {
        ...
        await toNetInput(undefined as never);
      } catch (error) {
    ...
    it('undefined at input index 1', async () => {
      ...
      try {
        // Intentionally pass invalid input to test error handling
        await toNetInput([env.getEnv().createImageElement(), undefined] as never);
      } catch (error) {
        errorMessage = (error as Error).message;
      }
      expect(errorMessage).toBe(
        'toNetInput - at input index 1: expected media to be of type HTMLImageElement | HTMLVideoElement | HTMLCanvasElement | tf.Tensor3D, or to be an element id'
      );
    });
  });
  ...
  describe('no memory leaks', () => {
    it('constructor', async () => {
      ...
      const tensor4ds = tensors.map(t => tf.expandDims(t, 0) as tf.Tensor4D);
