...
class FakeNeuralNetwork extends NeuralNetwork<FakeNetParams> {
  constructor(
  ...
  protected extractParams(_: Float32Array): {
...
describe('NeuralNetwork', () => {
  describe('getParamFromPath', () => {
    ...
    it('throws if param is not a tensor', () =>
      tf.tidy(() => {
        // Deliberately pass null to test error handling
        const net = new FakeNeuralNetwork(null as unknown as tf.Tensor);
        const fakePath = 'conv/filter';

        expect(() => net.getParamFromPath(fakePath)).toThrowError(
          `traversePropertyPath - parameter is not a tensor, for path ${fakePath}`
        );
      }));

    it('throws if key path invalid', () =>
      tf.tidy(() => {
        const net = new FakeNeuralNetwork();
        const fakePath = 'conv2d/foo';

        expect(() => net.getParamFromPath(fakePath)).toThrowError(
          `traversePropertyPath - object does not have property conv2d, for path ${fakePath}`
        );
      }));
  });

  describe('reassignParamFromPath', () => {
    it('sets correct params', () =>
      tf.tidy(() => {
        const net = new FakeNeuralNetwork();
    ...
    it('throws if param is not a tensor', () =>
      tf.tidy(() => {
        ...
        expect(() => net.reassignParamFromPath(fakePath, tf.tensor(0))).toThrowError(
          `traversePropertyPath - parameter is not a tensor, for path ${fakePath}`
        );
      }));
  ...
  describe('getParamList', () => {
    it('returns param tensors with path', () =>
      tf.tidy(() => {
        const convFilter = tf.tensor(0);
        const convBias = tf.tensor(0);
        const fcWeights = tf.tensor(0);
  ...
  describe('getTrainableParams', () => {
    it('returns all trainable params', () =>
      tf.tidy(() => {
